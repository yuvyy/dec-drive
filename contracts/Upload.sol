...
contract Upload {
  ...
  function add(address _user, string memory _name, string memory _hash) external {
    require(msg.sender == _user || ownership[_user][msg.sender], "You don't have access");
    File memory file = File(_name, _hash);
    userFiles[_user].push(file);
  }
